...
{
#if defined(__linux__)
    SetConsolePos(1, YLAST+rows.size()+2);
    printf("\n");
#else
#endif
}
...
int TableEngineConsole::CreateTable(const char *pColumnName[], unsigned nCount)
{
#if defined(__linux__)
    get_screen(&WS, &HS);
    get_pos(&XC, &YC);
...
int TableEngineConsole::CreateTable(const std::vector<std::string>& columns)
{
#if defined(__linux__)
    get_screen(&WS, &HS);
    get_pos(&XC, &YC);
    ...
    for(unsigned i=0; i<columns.size(); i++) {
        SetValueTable(0, i, "%s", columns[i].c_str());
    }
