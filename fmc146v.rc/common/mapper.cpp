...
int Mapper::openDevMem()
{
    ...
    //fprintf(stderr, "%s, %d, %s(): %s", __FILE__, __LINE__, __func__, "Open /dev/mem - OK\n");
    return 0;
}
...
void* Mapper::map(size_t physicalAddress, uint32_t areaSize)
{
    ...
    //fprintf(stderr, "%s, %d: %s() - Map 0x%x --> %p\n", __FILE__, __LINE__, __FUNCTION__, physicalAddress, map.virtualAddress);
