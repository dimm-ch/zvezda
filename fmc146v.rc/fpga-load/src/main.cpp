...
bool write_bitstream_data(brd_dev_t dev, S32 trd_idx, U32 fpga_mask, std::vector<uint16_t>& data)
{
    ...
    bool status = wait_status_bits_set(dev, fpga_mask, trd_idx, 10);
    if(!status) {
        fprintf(stderr, "%s(): Error wait TRD_STATUS_REG[CMD_RDY]!\n", __func__);
        return false;
    }
    ...
    status = wait_status_bits_clear(dev, (done_mask|init_mask), trd_idx, 10);
    if(!status) {
        fprintf(stderr, "%s(): Error wait TRD_STATUS_REG[STATUS_DONEx | STATUS_INITx]!\n", __func__);
        return false;
    }
    ...
    status = wait_status_bits_set(dev, init_mask, trd_idx, 10);
    if(!status) {
        fprintf(stderr, "%s(): Error wait TRD_STATUS_REG[STATUS_INITx]!\n", __func__);
        return false;
    }
    ...
    for(const auto& value : data) {
        ...
        //bool status = wait_status_bits_set(dev, 0x1, trd_idx, 1);
        //if(!status) {
        //    fprintf(stderr, "%s(): Error wait TRD_STATUS_REG[CMD_RDY]!\n", __func__);
        //    return false;
        //}
    ...
    if(!exit_flag) {
        ...
        status = wait_status_bits_set(dev, (done_mask|init_mask), trd_idx, 10);
        if(!status) {
            fprintf(stderr, "%s(): Error wait TRD_STATUS_REG[DONEx:INITx]!\n", __func__);
            return false;
        }
