...
brd_dev::brd_dev(unsigned lid) : m_lid(lid)
{
    m_hReg = 0;
...
{
    for(U32 j = 0; j<m_list.size(); j++) {
        const InSys::brd_string& srvName = m_list.at(j).c_str();
        if(srvName == InSys::brd_string(_BRDC("REG0"))) {
...
{
    if(m_hReg) BRD_release(m_hReg, 0);
}

//-----------------------------------------------------------------------------

BRD_Handle brd_dev::lock_service(const InSys::brd_string& name, U32 mode)
{
    for(U32 j = 0; j<m_list.size(); j++) {
        const InSys::brd_string& srvName = m_list.at(j).c_str();
        if(srvName == name) {
            BRD_Handle hSrv = BRD_capture(m_hBrd, 0, &mode, srvName.c_str(), 5000);
            if(hSrv <= 0) {
                fprintf(stderr,"%s, %d: %s() - Error in BRD_capture(): %s\n", __FILE__, __LINE__, __FUNCTION__, srvName.c_str());
                return 0;
            } else {
...
brd_stream::brd_stream(brd_dev_t dev, unsigned id, unsigned dir, unsigned src) :
    m_dev(dev), m_id(id), m_dir(dir), m_srcTrd(src)
{
    InSys::brd_string srv = InSys::brd_string(_BRDC("BASESTREAM")) + InSys::to_brd_string(id);
...
{
    cleanup();
    if(m_hStrm) m_dev->unlock_service(m_hStrm);
}
...
bool brd_stream::adjust(bool enable)
{
    ...
    {
        BRDC_printf( _BRDC("Handshake mode: 0x%.8X\r\n"), err );
        return false;
    }
    m_agree = enable;
    if(m_agree) {
...
{
    if(!m_hStrm) return nullptr;
    if(id >= m_blocks.size()) return nullptr;
...
brd_sdram::brd_sdram(brd_dev_t dev, unsigned id) : m_dev(dev), m_id(id)
{
    InSys::brd_string srv = InSys::brd_string(_BRDC("BASESDRAM")) + InSys::to_brd_string(id);
...
brd_sdram::~brd_sdram()
{
    if(m_hSDRAM) m_dev->unlock_service(m_hSDRAM);
}

//-----------------------------------------------------------------------------

bool brd_sdram::prepare(uint32_t size)
{
    U32 nFifoMode = 0;
...
bool brd_sdram::config()
{
    ...
    if(status < 0) {
        ...
        if(m_SdramConfig.MemType == 11) {
            m_PhysMemSize =	(unsigned)(((((__int64)m_SdramConfig.CapacityMbits * 1024 * 1024) >> 3) *
                                        (__int64)m_SdramConfig.PrimWidth / m_SdramConfig.ChipWidth * m_SdramConfig.ModuleBanks *
                                        m_SdramConfig.ModuleCnt) >> 2); // в 32-битных словах
        } else {
            m_PhysMemSize =	(1 << m_SdramConfig.RowAddrBits) *
                    (1 << m_SdramConfig.ColAddrBits) *
                    m_SdramConfig.ModuleBanks *
                    m_SdramConfig.ChipBanks *
                    m_SdramConfig.ModuleCnt * 2; // в 32-битных словах
        }
    }
