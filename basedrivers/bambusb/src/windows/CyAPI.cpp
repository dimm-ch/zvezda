...
bool CCyUSBDevice::Open(UCHAR dev){
    ...
    if (CreateHandle(dev)) {
        ...
        if ((BcdUSB & BCDUSBJJMASK) == USB30MAJORVER)
        {
            if(GetInternalBosDescriptor()) // USB3.0 specific descriptor
			{
				...
				catch(char *Str)
				{
					...
					MessageBox(NULL, L"Please correct firmware BOS descriptor table", L"Wrong BOS Descriptor", NULL);
        ...
        for (int i=0; i<Configs; i++)
		{
			...
			{
				char *Str1 = Str; // just to ignore warning
				MessageBox(NULL, L"Please correct firmware descriptor table", L"Wrong Device Configuration", NULL);
				Close(); // Close the device handle.
				return false;
			}
        }
