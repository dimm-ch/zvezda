...
//	FindKeyWord("dlgtype", m_pIniData->DlgDllName, pInitData, size);
	...
	for (int iMem = 0; iMem < MAX_MEMORY_SRV; iMem++)
	{
		...
		if(iMem)	BRDC_sprintf(key_name, _BRDC("memtetrnum%d"), iMem);
		else		BRDC_strcpy(key_name, _BRDC("memtetrnum"));
		...
		if (iMem)	BRDC_sprintf(key_name, _BRDC("memadrmask%d"), iMem);
		else		BRDC_strcpy(key_name, _BRDC("memadrmask"));
		FindKeyWord(key_name, m_pIniData->MemIni[iMem].AdrMask, pInitData, size);
		if (iMem)	BRDC_sprintf(key_name, _BRDC("memadrconst%d"), iMem);
		else		BRDC_strcpy(key_name, _BRDC("memadrconst"));
...
//			return DRVERR(BRDerr_BAD_DEVICE_VITAL_DATA);
		...
		if( m_DeviceID != AMBPEX8_DEVID &&
			...
			m_DeviceID != FMC131VKU_DEVID &&
			m_DeviceID != FMC131VZQ_DEVID &&
			m_DeviceID != FMC141VKU1_DEVID &&
			m_DeviceID != FMC141VKU2_DEVID &&
			m_DeviceID != FMC141VZQ_DEVID &&
			m_DeviceID != FMC143VKU_DEVID &&
			m_DeviceID != FMC143VZQ_DEVID &&
			m_DeviceID != FMC143VKUS_DEVID &&
			m_DeviceID != FMC146VKU_DEVID &&
			m_DeviceID != FMC146VZQ_DEVID &&
			m_DeviceID != FMC132P_DEVID &&
			m_DeviceID != FMC139P_DEVID &&
			m_DeviceID != FMC133V_DEVID &&
			m_DeviceID != DSP134V_DEVID &&
			m_DeviceID != FMC121P_DEVID &&
			m_DeviceID != RFG2_DEVID &&
			m_DeviceID != PANORAMA_DEVID &&
			m_DeviceID != D2XT006_DEVID &&
			m_DeviceID != FMC130E_DEVID
			) // все типы поддерживаемых модулей
		{
			if(m_hMutex)
			{
				IPC_deleteMutex(m_hMutex);
...
//			return DRVERR(BRDerr_BAD_DEVICE_VITAL_DATA);
			...
			m_DeviceID == FMC117CP_DEVID)
		{
			...
			if(m_pIniData)
			{
				...
				if((m_pIniData->PID != (U32)-1) && (m_pIniData->OrderNumber != (U32)-1))
				{ // к PID из ini-файла замешиваю order из ini-файла
					if(!m_pIniData->PID)
						m_pIniData->PID = m_PID;
					m_pIniData->PID += (m_pIniData->OrderNumber << 28);
				}
				if(m_pIniData->SlotNumber != (U32)-1)
				{ // идентификация по номеру слота (географическому адресу) и order'у
...
//	pBoard->m_PID    = PID;
	...
	BRDC_sprintf(m_name, _BRDC("CAMBPEX_%X"), m_PID);
...
ULONG CBambpex::InitSensorsFMC107()
{
	...
	for (i = 0; i < SENSOR_COUNT_FMC107P; i++)
	{
		...
		if (aPG_MAX[i] == 40) cfg_val[i].ByFields.PG = 0x0;
		if (aPG_MAX[i] == 80) cfg_val[i].ByFields.PG = 0x1;
		if (aPG_MAX[i] == 160) cfg_val[i].ByFields.PG = 0x2;
		if (aPG_MAX[i] == 320) cfg_val[i].ByFields.PG = 0x3;
...
ULONG CBambpex::InitSensorsFMC132(U32 devnum)
{
	...
	if (aPG_MAX == 40) cfg_val.ByFields.PG = 0x0;
	if (aPG_MAX == 80) cfg_val.ByFields.PG = 0x1;
	if (aPG_MAX == 160) cfg_val.ByFields.PG = 0x2;
	if (aPG_MAX == 320) cfg_val.ByFields.PG = 0x3;
...
ULONG CBambpex::GetSensorsFMC132(PBRDextn_Fmc132PSensors pSensors, U32 dev_num)
{
	...
	// и 1 микросхема LTC2991 (Voltage, Temperature and fan)
	{
		...
		U32 flten = 0;
		if (pSensors->mode)
		{ // режим 1 - это режим с усреднением (Filter Enabled)
			flten = 1;
			ctrl_val |= 0x8888;
		}
...
ULONG CBambpex::SetServices()
{
	...
	for(int iMem = 0; iMem < MAX_MEMORY_SRV; iMem++)
	{
		if (FMC126P_DEVID == m_DeviceID ||
			FMC139P_DEVID == m_DeviceID ||
			FMC131VKU_DEVID == m_DeviceID ||
			FMC141VKU1_DEVID == m_DeviceID ||
			FMC141VKU2_DEVID == m_DeviceID ||
			FMC143VKU_DEVID == m_DeviceID ||
			FMC146VKU_DEVID == m_DeviceID ||
			FMC132P_DEVID == m_DeviceID)
		{ // модули с DDR4
		...
		if(	(FMC106P_DEVID == m_DeviceID) ||
			(FMC107P_DEVID == m_DeviceID) ||
			//(PS_SYNC_DEVID == m_DeviceID) || // ВРЕМЕННЫЙ КОД - НЕ ЗАБЫТЬ УБРАТЬ !!!!!!!!!!!!!!!!!!!
			(FMC110P_DEVID == m_DeviceID) ||
			(FMC111P_DEVID == m_DeviceID) ||
			(FMC113E_DEVID == m_DeviceID) ||
			(FMC112CP_DEVID == m_DeviceID) ||
			(FMC114V_DEVID == m_DeviceID) ||
			(FMC117CP_DEVID == m_DeviceID) ||
			(FMC121CP_DEVID == m_DeviceID) ||
			(FMC121P_DEVID == m_DeviceID) ||
			(PANORAMA_DEVID == m_DeviceID) ||
            (FMC123E_DEVID == m_DeviceID) ||
			(FMC124P_DEVID == m_DeviceID) ||
			(FMC125CP_DEVID == m_DeviceID) ||
			(FMC127P_DEVID == m_DeviceID) ||
                        (FMC130E_DEVID == m_DeviceID) ||
			(FMC122P_DEVID == m_DeviceID) ||
			(XM416X250M_DEVID == m_DeviceID) ||
			(FMC115CP_DEVID == m_DeviceID) )
		{ // модули с DDR3
