//****************** File bambpex.h *******************************
// Definitions of structures, constants and class CBambpex
...
#define MAX_MEMORY_SRV 4

#define MAX_MEM_INI 4
...
class CBambpex : public CBaseModule
{
...
//	U32		m_PID;				// Physical Identifier (Serial Number) // CModule
	...
	PULONG	m_pMemBuf[3];		// виртуальные адреса аппаратных областей
	U32		m_PhysMem[3];		// физические адреса аппаратных областей
    ...
    IPC_handle	m_devMem;           // /dev/mem Driver Handle (for _c6x_ platform)
...
//	void GetSdramCfgICR(PVOID pCfgMem, ULONG RealBaseCfgSize, PICR_CfgDdrSdram pCfgSdram);
	...
	const BRDCHAR* getClassName(void) { return _BRDC("CBambpex"); }

public:

	CBambpex();
	CBambpex(PBRD_InitData pInitData, long sizeInitData);
	~CBambpex();
...
//	void operator delete (void* p) {if(p) HeapFree( GetProcessHeap(), 0, p ); }
	...
	virtual ULONG InitSensorsFMC107();
	virtual ULONG InitSensorsFMC132(U32 devnum);
	ULONG GetSensorsFMC132(PBRDextn_Fmc132PSensors pSensors, U32 dev_num);
	virtual ULONG GetSensors(void* sens);

	virtual ULONG setPWM(U32 dev_num, U32 dev_adr, U32 pwm_enbl, U32 pwm_inv, U32 pwm_thres);
	virtual ULONG setModeSensors(U32 dev_num, U32 dev_adr, U32 flt, U32 ctrl_reg6, U32 ctrl_reg7);
	virtual ULONG readSensors(U32 dev_num, U32 dev_adr, double* v_chan, double* tempr, double* vcc);

	ULONG WaitSemaReady(ULONG* BlockMem, int SemaOrReady);
	ULONG ReadMainSpd(int devid, USHORT* buf, USHORT Offset, int Length);
