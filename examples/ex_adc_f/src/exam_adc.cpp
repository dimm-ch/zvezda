...
try {
    ...
    auto one_cycle = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(one_cycle - adc_start_time);
    BRDC_printf(_BRDC("TIME=%lld \n"), adc_start_time);
    BRDC_printf(_BRDC("TIME=%lld \n"), one_cycle);
    BRDC_printf(_BRDC("TIME=%lld \n"), duration);
...
void ParseCommandLine(int argc, BRDCHAR* argv[])
{
    ...
    for (ii = 1; ii < argc; ii++) {
        if (argv[ii][0] != _BRDC('-')) {
            ...
            BRDC_printf(_BRDC("Command line: ini-filename %s\n"), g_iniFileName);
            continue;
        }
...
void DisplayHelp()
{
    ...
    BRDC_printf(_BRDC("\n"));
    BRDC_printf(_BRDC("adcf [ini-file] [options]\n"));
    BRDC_printf(_BRDC("\n"));
...
S32 GetAdcData(BRD_Handle hADC, unsigned long long bBufSize, unsigned long long bMemBufSize)
{
    ...
    using namespace std;
    std::ifstream fis;
    fis.open("regio_com.txt");
    commandLineParams paramReg;
    unsigned int locMode = BRDcapt_SHARED;
    paramReg.hService = BRD_capture(handle[iDev], 0, &locMode, "REG0", 5000);
    if (paramReg.hService < 0)
        printf("-*- Service REG0 dont found .. \n");
    else {
        string line;
