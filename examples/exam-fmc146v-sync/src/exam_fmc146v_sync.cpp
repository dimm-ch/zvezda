...
class CExamFmc146vSyncOptions final : public CExamOptions {
public:
    CExamFmc146vSyncOptions(int argc, char* argv[])
    {
        _description.add_options()("exam-ini",
            po::value<std::string>()->default_value(g_SettingsFileNameDefault),
            "Exam configuration file");
        _description.add_options()("brd-ini",
            po::value<std::string>()->default_value(g_BrdIniFileNameDefault),
            "Bardy configuration file");
        parseOptions(argc, argv);
    }
    std::string exam_ini() const { return _vm["exam-ini"].as<std::string>(); }
    std::string brd_ini() const { return _vm["brd-ini"].as<std::string>(); }
};
...
int BRDC_main(int argc, char* argv[])
{
    ...
    // устанавливаем обработчики аварийного завершения программы
    ...
    return exceptCatcher([&] {
        ...
        if (examOptions.version().has_value()) {
            std::cout << examOptions.version().value() << std::endl;
            return EXIT_SUCCESS;
        }
        if (examOptions.help().has_value()) {
            std::cout << examOptions.help().value() << "\n";
            return EXIT_SUCCESS;
        }
        color::set_enabled(!examOptions.nocolor());
        auto verboseInfo = examOptions.verbose();
        CONSOLE_CLEAR();
        ...
        // инициализируем управляющую бибилиотеку
        auto brdIni = to_brd_string(examOptions.brd_ini());
        CBardy::init(brdIni);
        ...
        const auto deviceInfoList = CBardy::getDeviceInfoList();
        // список устройств
        ...
        ERROR_ASSERT(deviceList.empty() != true, "No devices found!");
        ...
        ERROR_ASSERT(fmc146vSyncList.empty() != true, "Unit SYNC not found!");
        ...
        INIReader settingsFile { examOptions.exam_ini() };
        ERROR_ASSERT((settingsFile.ParseError() == 0), "Can't read settings file!");
        // основной цикл
    //    do {
            // проверяем было ли нажатие клавиатуры
   //         const auto isPressed = keyboard.isKeyPressed();
           /* if (isPressed) {
                // получаем код нажатой клавиши
                auto keyCode { keyboard.getKeyCode() };
                switch (keyCode) {
                // запрос на выполнения синхронизации
                case CKeyboard::KeyCode_S:
                    requestSyncronize = true;
                    break;
                // запрос на прерывание теста
                case CKeyboard::KeyCode_Escape:
                    requestCancel = true;
                    break;
                // запрос на обновление параметров из файла инициализации
                case CKeyboard::KeyCode_Space:
                    requestSyncronize = true;
                    requestUpdate = true;
                    CONSOLE_CLEAR();
                    break;
                ...
                case 81: // Q
                    // FIXME: >>>
                    ++tmp_delayADEL2;
                    if (tmp_delayADEL2 > 63) {
                        tmp_delayADEL2 = 0;
                    }
                    for (auto& fmc146vSync : fmc146vSyncList) {
                        fmc146vSync->LTC6953.set_analog_delay({ tmp_outputADEL, tmp_delayADEL2 });
                    }
                    color::printf("{+white}Set ADEL2{} = {+yellow}%3.1d{}\r", tmp_delayADEL2);
                    // FIXME: <<
                    break;
                case 65: // A
                    // FIXME: >>>
                    ...
                    for (auto& fmc146vSync : fmc146vSyncList) {
                        fmc146vSync->LTC6953.set_analog_delay(
                            { tmp_outputADEL, tmp_delayADEL2 });
                    }
                    color::printf("{+white}Set ADEL2{} = {+yellow}%3.1d{}\r", tmp_delayADEL2);
                    // FIXME: <<
                    break;
            ...
            if (requestUpdate) {
                ...
                for (auto& fmc146vSync : fmc146vSyncList) {
                    ...
                    ERROR_ASSERT((retFMODE.getValue() == regFMODE.getValue()), "Write FMODE register error!");
                    ...
                    // проверяем успешность обновления параметра
                    ERROR_ASSERT((retDAC.getValue() == regDAC.getValue()), "Write DAC register error!");
                    ...
                    // проверяем успешность обновления параметра
                    ERROR_ASSERT((retMUX.getValue() == regMUX.getValue()), "Write MUX register error!");
                    ...
                    // проверяем успешность обновления параметра
                    ERROR_ASSERT((retFSRC.getValue() == regFSRC.getValue()), "Write FSRC register error!");
                    ...
                    // проверяем успешность обновления параметра
                    ERROR_ASSERT((retFSRC3.getValue() == regFSRC3.getValue()), "Write FSRC3 register error!");
            ...
            if (currentTime > nextTime) {
                ...
                for (auto& fmc146vSync : fmc146vSyncList) {
                    ...
                    ERROR_ASSERT(regSTATUS->Bits.ALARM == false,
                        "Warning! FPGA temp. > 80C");
                    ERROR_ASSERT(regSTATUS->Bits.OT == false, "Alert! FPGA temp. > 85C");
                }
            }
