...
S32 SdramSettings(ULONG mem_mode, BRD_Handle hADC, uint64_t& bBuf_size)
{
	...
	BRD_ctrl(hADC, 0, BRDctrl_SDRAM_GETCFGEX, &rSdramConfig);
	...
	else
	{
		...
		if (rSdramConfig.PrimWidth == 32) {
			mem_size = (uint64_t)(bBuf_size >> 1);
		}
		else
			if (rSdramConfig.PrimWidth == 64) {
				mem_size = uint64_t(bBuf_size >> 2); // получить размер активной зоны в 32-разрядных словах
			} 
