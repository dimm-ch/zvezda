...
//	BRDctrl_ADC_GETCLKTUNING	= BRDctrl_ADC + 51,	// Get ADC clock frequency tuning in % (REAL64)
	...
	BRDctrl_ADC_SETPKGRATE			= BRDctrl_ADC + 145, // Установка степени сжатия (U32)
	BRDctrl_ADC_SETPKGTHRESHOLD	= BRDctrl_ADC + 146, // Установка порога отсечения шума (S32)
	BRDctrl_ADC_SETPKGDURATION		= BRDctrl_ADC + 147, // Установка длительности регистрации отсчетов (U32)
	BRDctrl_ADC_SETPKGOFFSET		= BRDctrl_ADC + 148, // Установка отступа от запускающего импульса (U32)
	BRDctrl_ADC_RESETPKGCOUNT		= BRDctrl_ADC + 149, // Сброс счетчика регистраций
	BRDctrl_ADC_SETPKGENABLE		= BRDctrl_ADC + 150, // Установка степени сжатия (U32)
	BRDctrl_ADC_SETPKGGEN			= BRDctrl_ADC + 151, // Управление генератором (PBRD_AdcPkgGen)
...
typedef struct _BRD_AdcPkgGen {
	U32		nGenPeriod;
	U32		nGenWidth;
	U32		nGenEn;
} BRD_AdcPkgGen, *PBRD_AdcPkgGen;
