...
// Numbers of DDR4 SPD Registers
typedef enum _DDR4_NUM_SPD_BYTES {
	DDR4spd_MEMTYPE = 2,	// DRAM Device Type: 0C - DDR4 SDRAM
...
typedef enum _SDRAM_NUM_CMD_REGS {
	...
	SDRAMnr_ADDRMASK	= 22, // 0x16
	SDRAMnr_ADDRCONST	= 23, // 0x17
} SDRAM_NUM_CMD_REGS;

// Numbers of Constant Registers
typedef enum _SDRAM_NUM_CONST_REGS {
	SDRAMnr_RDBURST = 0x108,
	SDRAMnr_WRBURST = 0x109,
	SDRAMnr_CNTRID = 0x10A, // CONTROLLER_ID
	SDRAMnr_COUPLE = 0x10B, // COUPLE_TRD_NUMBER
} SDRAM_NUM_CONST_REGS;
...
//	SDRAMnr_SYNCEVENTH	= 0x205,
	SDRAMnr_RDSTART		= 0x20F,	// 1- запуск чтения активной зоны (для тестов через две тетрады памяти)
