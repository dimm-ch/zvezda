...
// div:     делитель [0..255]
auto LMX2594::set_input_path(double freq, bool x2, size_t pre_div, size_t mult, uint8_t div) -> void
{
    fosc = freq;
    regs.OSC_2X = x2;
    ...
    LOG("VCO Calibration: FPD Freq = %.0f MHz, LPFD_ADJ = '%s', HPFD_ADJ = '%s', CAL_CLK_DIV = '%s'",
        fpd,
        (regs.FCAL_LPFD_ADJ == 0) ? "Fpd >= 10" : (regs.FCAL_LPFD_ADJ == 1) ? "5 >= Fpd > 10"
            : (regs.FCAL_LPFD_ADJ == 2)                                     ? "2.5 >= Fpd > 5"
            : (regs.FCAL_LPFD_ADJ == 3)                                     ? "Fpd < 2.5"
                                                                            : "NONE",

        (regs.FCAL_HPFD_ADJ == 0) ? "Fpd <= 100" : (regs.FCAL_HPFD_ADJ == 1) ? "100 < Fpd <= 150"
            : (regs.FCAL_HPFD_ADJ == 2)                                      ? "150 <= Fpd =< 200"
            : (regs.FCAL_HPFD_ADJ == 3)                                      ? "Fpd > 200"
                                                                             : "NONE",

        (regs.CAL_CLK_DIV == 0) ? "Fosc <= 200" : (regs.CAL_CLK_DIV == 1) ? "200 < Fosc <= 400"
...
auto LMX2594::get_fraction() -> size_t
{
