...
auto SrvADC::CtrlGetCfg(void* pDev, void* pServData, ULONG cmd, void* args) -> int
{
    ...
    // параметры АЦП
    param.Bits = m_adc.is_complex() ? 16 : m_bits; // разрядность: ADC = 14, DDC = 16 бит
...
auto SrvADC::CtrlReadIniFile(void* pDev, void* pServData, ULONG cmd, void* args) -> int
{
    ...
    auto input = BRD_DdcInputIQ_FM214x3GDA {};
    for (auto i = 0U; i < 4; i++) {
        input.channel = i;
    ...
    auto fd = BRD_FastDetect_FM214x3GDA {};
    for (auto i = 0U; i < 4; i++) {
        fd.channel = i;
...
auto SrvADC::CtrlGetSyncMode(void* pDev, void* pServData, ULONG cmd, void* args) -> int
{
    ...
    sync_mode.rate = m_adc.get_sample_rate() * 1'000'000;
...
auto SrvADC::CtrlGetTraceText(void* pDev, void* pServData, ULONG cmd, void* args) -> int
{
    ...
    BRDC_snprintf(textbuf.pText, textbuf.sizeb, _BRDC("ADC_OVR:%s"), InSys::to_brd_string(adc_ovr).c_str());

    // Монитор LTC
    // auto data = sub_module.m_monitor.get_data();
    // BRDC_snprintf(textbuf.pText, textbuf.sizeb, _BRDC("T:%.02f V6:%.02f V7:%.02f V8:%.02f"),
    //     data.Tint, data.V6, data.V7 * 2, data.V8 * 3);
