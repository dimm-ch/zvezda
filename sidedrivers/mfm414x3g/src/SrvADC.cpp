...
/// Конструктор
///
SrvADC::SrvADC(Cmfm414x3g* parent)
    : CService(-1, _BRDC("ADC414x3G"), -1, parent, nullptr, 0)
{
    ...
    m_adc1.setup_spi(
        [this, parent](size_t reg) -> uint8_t { return parent->SpdRead(m_tetr_num, 0, 1, reg); },
        [this, parent](size_t reg, uint8_t val) -> void { parent->SpdWrite(m_tetr_num, 0, 1, reg, val); });
    ...
    m_Cmd.push_back(new CMD_Info { BRDctrl_ADC_SETTESTSEQUENCE, 0, 0, 0, CmdEntry(&SrvADC::CtrlSetTestSequence) });
    ...
    m_Cmd.push_back(new CMD_Info { BRDctrl_ADC_GETBLKCNT, 0, 0, 0, CmdEntry(&SrvADC::CtrlGetBlkCount) });
    m_Cmd.push_back(new CMD_Info { BRDctrl_ADC_SETEVENTCNT, 0, 0, 0, CmdEntry(&SrvADC::CtrlSetEventCount) });
    ...
    m_Cmd.push_back(new CMD_Info { BRDctrl_ADC_PREPARESTART, 0, 0, 0, CmdEntry(&SrvADC::CtrlPrepareStart) });
...
auto SrvADC::CtrlIsAvailable(void* pDev, void* pServData, ULONG cmd, void* args) -> int
{
    ...
    // для добавления специфичных команд/функций проекта "Криотрейд",
    // используем номер модификации тетрады
    m_cryo = (sub_module.IndRegRead(m_tetr_num, ADM2IFnr_IDMOD) == 0x4B54); // 'KT'
    if (m_cryo) {
        m_Cmd.push_back(new CMD_Info { BRDctrl_ADC_SETCOMPENABLE, 0, 0, 0, CmdEntry(&SrvADC::CtrlSetCompEnable) });
        m_Cmd.push_back(new CMD_Info { BRDctrl_ADC_GETCOMPENABLE, 0, 0, 0, CmdEntry(&SrvADC::CtrlGetCompEnable) });
        m_Cmd.push_back(new CMD_Info { BRDctrl_ADC_SETCOMPLEVEL, 0, 0, 0, CmdEntry(&SrvADC::CtrlSetCompLevel) });
        m_Cmd.push_back(new CMD_Info { BRDctrl_ADC_GETCOMPLEVEL, 0, 0, 0, CmdEntry(&SrvADC::CtrlGetCompLevel) });
        m_Cmd.push_back(new CMD_Info { BRDctrl_ADC_SETCOMPINVERSE, 0, 0, 0, CmdEntry(&SrvADC::CtrlSetCompInverse) });
        m_Cmd.push_back(new CMD_Info { BRDctrl_ADC_GETCOMPINVERSE, 0, 0, 0, CmdEntry(&SrvADC::CtrlGetCompInverse) });
    }
...
///
/// \brief      Получение маски каналов ADC/DDC. Регистр ADM2IFnr_CHAN1 [0x10] и 0x11
...
///
/// \brief      Установка и инициализация источника тактовой частоты для АЦП.
///
/// \param      pDev       The development
/// \param      pServData  The serv data
/// \param      cmd        The command
...
{
    decltype(auto) sub_module = ModuleCast<Cmfm414x3g>(pDev);
    decltype(auto) clk_mode = ArgumentCast<BRD_ClkMode_FM414x3G>(args);
    auto ctrl = CONTROL1_0x17 {};

    // Инициализация LMX2594 [ SYNC ]
    // -------------------------------------------------------------------------
    // TODO: проверить на уже запущеный LMX
    sub_module.SetClkMode(clk_mode.src, clk_mode.Fout, clk_mode.Fosc,
        clk_mode.PwrA, clk_mode.PwrB, clk_mode.OutA, clk_mode.OutB);

    // выключим все микросхемы
    // ctrl.all = sub_module.IndRegRead(m_tetr_num, 0x17);
    // ctrl.PLL_CE = false; // disable LMX
    // ctrl.ADC0_PW = false; // disable AD9208 #0
    // ctrl.ADC1_PW = false; // disable AD9208 #1
    ...
    // AD9208 #0
    // -------------------------------------------------------------------------
    ctrl.PWUP_ADC0 = true; // включим питание ADC0
    sub_module.IndRegWrite(m_tetr_num, 0x17, ctrl.all);
    m_adc0.reset();
    m_adc0.get_chip_id();
    m_adc0.set_fclk(clk_mode.Fout);

    // AD9208 #1
    // -------------------------------------------------------------------------
    ctrl.PWUP_ADC1 = true; // включим питание ADC1
    sub_module.IndRegWrite(m_tetr_num, 0x17, ctrl.all);
    m_adc1.reset();
    m_adc1.get_chip_id();
    m_adc1.set_fclk(clk_mode.Fout);
...
{
    decltype(auto) sub_module = ModuleCast<Cmfm414x3g>(pDev);
    decltype(auto) mode = ArgumentCast<BRD_AdcMode_FM414x3G>(args);
    ...
    m_adc0.set_mode(mode.app_mode, mode.ddc_decimate);
    m_adc1.set_mode(mode.app_mode, mode.ddc_decimate);
...
auto SrvADC::CtrlSetJesd(void* pDev, void* pServData, ULONG cmd, void* args) -> int
{
    ...
    // -------------------------------------------------------------------------
    auto ctrl_sync = Cmfm414x3g::CONTROL1_0x17 {};
    ctrl_sync.all = sub_module.IndRegRead(sub_module.m_sync_tetr_num, 0x17);
    ctrl_sync.EXT_JESD_CORECLK = ext_core_clk;
    ...
    LOG("JESD Phy ID: %s [%02zXh]", m_jesd_phy_id == 0x10 ? "QPLL0" : "CPLL", m_jesd_phy_id);

    switch (m_jesd_phy_id) {
    case 0x10: // QPLL JESD204B
    case 0x18: // QPLL JESD204C
        if (m_adc0.set_jesd(15'500.0) == false || m_adc1.set_jesd(15'500.0) == false)
            return BRDerr_ERROR;
        break;
    case 0x12: // CPLL
        if (m_adc0.set_jesd(12'500.0) == false || m_adc1.set_jesd(12'500.0) == false)
            return BRDerr_ERROR;
        break;
    default:
    ...
    // тактовая частота АЦП до входа в LMK принудительно делится на 2
    // это необходимо учитывать при расчёте делителей LMK
    //
    using LMK_R0 = union {
        uint32_t all;
        struct {
    ...
    // BitRate = (20 * (Fclk / decimation) * M) / L
    // CoreCLK = BitRate / 40
    // RefCLK  = BitRate / 40 (должен находиться в диапазоне QPLL, CPLL и т.д.)
    r0.RESET = true;
    sub_module.SpdWrite(sub_module.m_sync_tetr_num, 3, 0, 0x000, r0.all);
    r0.RESET = false;
    // r0.CLKout8_11_PD = true;
    // r0.CLKout12_13_PD = true;
    r0.CLKin0_BUF_TYPE = 0; // Bipolar (LVDS)
    r0.CLKin1_BUF_TYPE = 0; // Bipolar (LVDS)

    // BitRate = (20 * (Fclk / decimation) * M) / L = (20 * (2900 / 2) * 2) / 8 = 7250
    // CoreCLK = BitRate / 40 = 7250 / 40 = 181.25
    // RefCLK  = BitRate / 40 = 7250 / 40 = 181.25
    // Divider = Fclk / CoreCLK = 2900 / 181.25 = 16
    // Divider = Divider / 2 = 8 (учёт аппаратного делителя)

    // Расчёт делителей
    auto found = false;
    auto core_clk = m_adc0.get_lane_rate() / 40;
    int common_div = m_adc0.get_fclk() / (core_clk * 2);

    for (auto div1 = 1; div1 <= 8 && !found; div1++)
        for (auto div2 = 1; div2 <= 8; div2++)
            if (div1 * div2 == common_div) {
                found = true;
                // MUX входного делителя
                if (div1 == 1)
                    r0.CLKin0_MUX = 0; // bypass
                else
                    r0.CLKin0_MUX = 1; // divide
                // входной делитель
                if (div1 == 8)
                    r0.CLKin0_DIV = 0; // 0 = div8
                else
                    r0.CLKin0_DIV = div1; // : 2..7 = div2..7
                // выходные делители
                if (div2 == 8) {
                    r5.CLKout0_3_DIV = 0; // 0 = div8
                    r5.CLKout4_7_DIV = 0; // 0 = div8
                    r5.CLKout8_11_DIV = 0;
                    r5.CLKout12_13_DIV = 0;
                } else {
                    r5.CLKout0_3_DIV = div2; // 1..7 = div1..7
                    r5.CLKout4_7_DIV = div2; // 1..7 = div1..7
                    r5.CLKout8_11_DIV = div2; // 1..7 = div1..7
                    r5.CLKout12_13_DIV = div2; // 1..7 = div1..7
                }
                break;
            }

    sub_module.SpdWrite(sub_module.m_sync_tetr_num, 3, 0, 0x000, r0.all);
    sub_module.SpdWrite(sub_module.m_sync_tetr_num, 3, 0, 0x003, r3.all);
    sub_module.SpdWrite(sub_module.m_sync_tetr_num, 3, 0, 0x005, r5.all);

    LOG("CoreCLK: %.02f MHz, RefCLK: %.02f MHz [ LMK Dividers: in %d, out %d,%d ]",
        core_clk, core_clk, r0.CLKin0_DIV, r5.CLKout0_3_DIV, r5.CLKout4_7_DIV);

    // JESD FPGA PHY's QPLL/CPLL GTX Common
    // ------------------------------------
    if ((m_jesd_phy_id == 0x10) || (m_jesd_phy_id == 0x18))
        sub_module.setup_jesd_qpll0(m_adc0.get_lane_rate());
    else // jesd_phy_id == 0x12
        sub_module.setup_jesd_cpll(m_adc0.get_lane_rate());

    // JESD FPGA приёмники
    // -------------------------------------------------------------------------
    auto jesd_L = m_adc0.get_jesd_L();

    auto ctrl = CONTROL1_0x17 {};
    ctrl.all = sub_module.IndRegRead(m_tetr_num, 0x17);
    ctrl.RX_RESET = 1; // взведём reset,
    sub_module.IndRegWrite(m_tetr_num, 0x17, ctrl.all);
    ...
    for (auto dev : JESD_FPGA_devs) {
        // проверка по маске каналов ADC/DDC количество используемых JESD'ов
        auto chan1 = ADM2IF_CHAN1 {};
        chan1.AsWhole = sub_module.IndRegRead(m_tetr_num, ADM2IFnr_CHAN1); // 0x10
        if (chan1.ByBits.ChanSel != 0) { // маска ADC
            if (((chan1.ByBits.ChanSel & 0b1100) == 0) && (dev == 9))
                continue; // пропускаем второй JESD
        } else { // маска DDC
            chan1.AsWhole = sub_module.IndRegRead(m_tetr_num, ADM2IFnr_CHAN1 + 1); // 0x11
            if (((chan1.ByBits.ChanSel & 0b11110000) == 0) && (dev == 9))
                continue; // пропускаем второй JESD
        }

        if (m_jesd_phy_id == 0x18) {
            // параметры JESD204C
            ...
            reg_03C |= m_adc0.get_jesd_F() - 1; // Octets per Frame - 1
            ...
            sub_module.SpdWrite(m_tetr_num, dev, 0, 0x028, // lanes in use
                (jesd_L == 1) ? 0b00000001 : (jesd_L == 2) ? 0b00000011
                    ...
                    : (jesd_L == 8)                        ? 0b11111111
                                                           : 0xF);
            sub_module.SpdWrite(m_tetr_num, dev, 0, 0x020, m_adc0.get_jesd_F() - 1); // Octets per Frame - 1
...
auto SrvADC::CtrlSetDDCFc(void* pDev, void* pServData, ULONG cmd, void* args) -> int
{
    ...
    if (val_chan.chan < 4)
        m_adc0.set_nco(val_chan.chan, val_chan.value);
    else
        m_adc1.set_nco(val_chan.chan - 4, val_chan.value);
...
auto SrvADC::CtrlGetDDCFc(void* pDev, void* pServData, ULONG cmd, void* args) -> int
{
    ...
    val_chan.value = (val_chan.chan < 4) ? m_adc0.get_nco(val_chan.chan) * 1'000'000 : m_adc1.get_nco(val_chan.chan - 4) * 1'000'000; // в Гц
...
auto SrvADC::CtrlSetDDCFcPhase(void* pDev, void* pServData, ULONG cmd, void* args) -> int
{
    ...
    if (val_chan.chan < 4)
        m_adc0.set_nco_offset(val_chan.chan, val_chan.value);
    else
        m_adc1.set_nco_offset(val_chan.chan - 4, val_chan.value);
...
{
    decltype(auto) sub_module = ModuleCast<Cmfm414x3g>(pDev);
    decltype(auto) val = ArgumentCast<BRD_DdcInputIQ_FM414x3G>(args);

    LOG("BRDctrl_DDC_SETINPSRC | channel: %zd, I: ADC%zd, Q: ADC%zd", val.channel, val.src_i, val.src_q);

    auto result = (val.channel < 4) ? m_adc0.set_ddc_input_iq(val.channel, val.src_i, val.src_q) : m_adc1.set_ddc_input_iq(val.channel - 4, val.src_i - 2, val.src_q - 2);
...
auto SrvADC::CtrlSetInvertChannelA(void* pDev, void* pServData, ULONG cmd, void* args) -> int
{
    ...
    m_adc0.set_invert_channel_a();
    m_adc1.set_invert_channel_a();
...
auto SrvADC::CtrlSetInputFSVoltage(void* pDev, void* pServData, ULONG cmd, void* args) -> int
{
    ...
    if (val.chan < 2)
        m_adc0.set_input_fs_voltage(val.chan, val.value);
    else
        m_adc1.set_input_fs_voltage(val.chan - 2, val.value);
...
auto SrvADC::CtrlSetInputBufferControl(void* pDev, void* pServData, ULONG cmd, void* args) -> int
{
    ...
    if (val.chan < 2)
        m_adc0.set_input_buffer_control(val.chan, size_t(val.value));
    else
        m_adc1.set_input_buffer_control(val.chan - 2, size_t(val.value - 2));
...
{
    decltype(auto) sub_module = ModuleCast<Cmfm414x3g>(pDev);
    decltype(auto) val = ArgumentCast<BRD_StartLevel_FM414x3G>(args);
...
{
    decltype(auto) sub_module = ModuleCast<Cmfm414x3g>(pDev);
    decltype(auto) val = ArgumentCast<BRD_SysRefConfig_FM414x3G>(args);
    ...
    LOG("BRDctrl_ADC_SETSYSREF | level: %.02f V, config: 0x%X", val.level, val.select);

    sub_module.SetSysRef(val.level, val.select);
...
auto SrvADC::CtrlSetClockDelayControl(void* pDev, void* pServData, ULONG cmd, void* args) -> int
{
    ...
    m_adc0.set_clock_delay_control(val);
    m_adc1.set_clock_delay_control(val);
...
{
    decltype(auto) sub_module = ModuleCast<Cmfm414x3g>(pDev);
    decltype(auto) params = ArgumentCast<BRD_ClockDelayChannel_FM414x3G>(args);
    ...
    if (params.channel < 2)
        m_adc0.set_clock_delay(params.channel, params.divider_phase, params.superfine_delay, params.fine_delay);
    else
        m_adc1.set_clock_delay(params.channel - 2, params.divider_phase, params.superfine_delay, params.fine_delay);
...
{
    decltype(auto) sub_module = ModuleCast<Cmfm414x3g>(pDev);
    decltype(auto) params = ArgumentCast<BRD_FastDetect_FM414x3G>(args);
    ...
    if (params.channel < 2)
        m_adc0.set_fast_detect(params.channel, params.enable, params.threshold_upper, params.threshold_lower, params.dwell_time);
    else
        m_adc1.set_fast_detect(params.channel - 2, params.enable, params.threshold_upper, params.threshold_lower, params.dwell_time);
...
auto SrvADC::CtrlSetLMFCCount(void* pDev, void* pServData, ULONG cmd, void* args) -> int
{
    ...
    sub_module.IndRegWrite(m_tetr_num, 0x1A, (/*jesd_K*/ 32 * m_adc0.get_jesd_F()) >> 2);
...
auto SrvADC::CtrlGetLMFCCount(void* pDev, void* pServData, ULONG cmd, void* args) -> int
{
    ...
    value->count = sub_module.IndRegRead(m_tetr_num, 0x218); // LMFC_COUNT_GET
    value->K = 32;
    value->F = m_adc0.get_jesd_F();
...
///
/// \brief  Получить номер текущего блока данных (кадра)
///
auto SrvADC::CtrlGetBlkCount(void* pDev, void* pServData, ULONG cmd, void* args) -> int
{
    decltype(auto) sub_module = ModuleCast<Cmfm414x3g>(pDev);
    decltype(auto) value = ArgumentCast<uint32_t>(args);
    ...
    value = (sub_module.IndRegRead(m_tetr_num, 0x211) << 16) | sub_module.IndRegRead(m_tetr_num, 0x210);
    LOG("BRDctrl_ADC_GETBLKCOUNT | Count: %d", value);

    return BRDerr_OK;
}

///
/// \brief  Установка/сброс счётчика внешнего события
///
auto SrvADC::CtrlSetEventCount(void* pDev, void* pServData, ULONG cmd, void* args) -> int
{
    decltype(auto) sub_module = ModuleCast<Cmfm414x3g>(pDev);
    decltype(auto) value = ArgumentCast<uint32_t>(args);
    ...
    sub_module.IndRegWrite(m_tetr_num, 0x20F, value >> 16);
    sub_module.IndRegWrite(m_tetr_num, 0x20E, value & 0xFFFF);
    LOG("BRDctrl_ADC_SETEVENTCOUNT | Count: %d", value);

    return BRDerr_OK;
}

///
/// \brief      Установка режима предварительной синхронизации JESD'ов
///
/// \param      pDev       The dev
/// \param      pServData  The serv data
/// \param      cmd        The command
...
{
    decltype(auto) sub_module = ModuleCast<Cmfm414x3g>(pDev);
    decltype(auto) value = ArgumentCast<BRD_PreSync_FM414x3G>(args);
    ...
    if (value.enable == false) {
        presync.MODE = size_t(BRD_PRESYNC_TYPE_FM414x3G::SYNC);
        presync.START = false;
    ...
    presync.MODE = size_t(BRD_PRESYNC_TYPE_FM414x3G::ASYNC);
    presync.START = true;
    sub_module.IndRegWrite(m_tetr_num, 0x18, presync.all);
    presync.START = false;
    sub_module.IndRegWrite(m_tetr_num, 0x18, presync.all);
...
{
    decltype(auto) sub_module = ModuleCast<Cmfm414x3g>(pDev);
    decltype(auto) value = ArgumentCast<BRD_TimeStamp_FM414x3G>(args);
    auto ctrl = CONTROL1_0x17 {};
    ...
    auto presync = PRESYNC_0x18 {};
    presync.all = sub_module.IndRegRead(m_tetr_num, 0x18);
    if (value.enable && (presync.MODE == size_t(BRD_PRESYNC_TYPE_FM414x3G::SYNC))) {
    ...
    auto ctrl_sync = Cmfm414x3g::CONTROL1_0x17 {};
    ctrl_sync.all = sub_module.IndRegRead(sub_module.m_sync_tetr_num, 0x17);
    ctrl_sync.SYSREF_ADC_CNF = value.enable ? 0b11 : ctrl_sync.SYSREF_ADC_CNF; // 11 -  Внешний старт на SYSREF АЦП, 00 - Сигнал SYSREF_ADC на АЦП
    sub_module.IndRegWrite(sub_module.m_sync_tetr_num, 0x17, ctrl_sync.all);
    ...
    m_adc0.set_time_stamp(value.enable, value.polarity_negative);
    m_adc1.set_time_stamp(value.enable, value.polarity_negative);
...
auto SrvADC::CtrlSetPretrigMode(void* pDev, void* pServData, ULONG cmd, void* args) -> int
{
    ...
    auto presync = PRESYNC_0x18 {};
    presync.all = sub_module.IndRegRead(m_tetr_num, 0x18);
    if (presync.MODE == size_t(BRD_PRESYNC_TYPE_FM414x3G::SYNC)) {
    ...
    auto sample_size = std::ceil(m_bits / 8.0) * (m_adc0.is_complex() ? 2 : 1); // в байтах
    ...
    double precount_min {};
    if (!m_cryo)
        precount_min = 2; // в общем случае не должно быть меньше двух слов FIFO
    if (precount < precount_min)
        precount = precount_min;
...
///
/// \brief Включение/выключение цифровых компараторов, 0 - выкл., 1 - вкл.
///
auto SrvADC::CtrlSetCompEnable(void* pDev, void* pServData, ULONG cmd, void* args) -> int
{
    decltype(auto) sub_module = ModuleCast<Cmfm414x3g>(pDev);
    decltype(auto) val_chan = ArgumentCast<BRD_ValChan>(args);
    auto ctrl = COMP_0x224 {};

    if (val_chan.chan > 3)
        val_chan.chan = 3;

    ctrl.all = sub_module.IndRegRead(m_tetr_num, 0x224);

    LOG("BRDctrl_ADC_SETCOMPENABLE | ch: %d, enable: %s", val_chan.chan, val_chan.value ? "ON" : "OFF");

    switch (val_chan.chan) {
    case 0:
        ctrl.COMP0_EN = bool(val_chan.value);
        break;
    case 1:
        ctrl.COMP1_EN = bool(val_chan.value);
        break;
    case 2:
        ctrl.COMP2_EN = bool(val_chan.value);
        break;
    case 3:
        ctrl.COMP3_EN = bool(val_chan.value);
        break;
    }

    sub_module.IndRegWrite(m_tetr_num, 0x224, ctrl.all);
    return BRDerr_OK;
}

auto SrvADC::CtrlGetCompEnable(void* pDev, void* pServData, ULONG cmd, void* args) -> int
{
    decltype(auto) sub_module = ModuleCast<Cmfm414x3g>(pDev);
    decltype(auto) val_chan = ArgumentCast<BRD_ValChan>(args);
    auto ctrl = COMP_0x224 {};
    ...
    switch (val_chan.chan) {
    case 0:
        val_chan.value = bool(ctrl.COMP0_EN);
        break;
    case 1:
        val_chan.value = bool(ctrl.COMP1_EN);
        break;
    case 2:
        val_chan.value = bool(ctrl.COMP2_EN);
        break;
    case 3:
        val_chan.value = bool(ctrl.COMP3_EN);
        break;
    }

    LOG("BRDctrl_ADC_GETCOMPENABLE | ch: %d, enable: %s", val_chan.chan, val_chan.value ? "ON" : "OFF");

    return BRDerr_OK;
}

///
/// \brief Установка амплитуды порога цифровых компараторов, диапазон допустимых значений от -1.4 до 1.4 (В)
///
auto SrvADC::CtrlSetCompLevel(void* pDev, void* pServData, ULONG cmd, void* args) -> int
{
    decltype(auto) sub_module = ModuleCast<Cmfm414x3g>(pDev);
    decltype(auto) val_chan = ArgumentCast<BRD_ValChan>(args);
    ...
    if (val_chan.value < -m_inp_range)
        val_chan.value = -m_inp_range;
    if (val_chan.value > m_inp_range)
        val_chan.value = m_inp_range;

    auto reg_value = uint16_t(std::round(val_chan.value * (std::pow(2, 16) - 2) / (m_inp_range * 2)));

    LOG("BRDctrl_ADC_SETCOMPLEVEL | ch: %d, level: %.02f V, reg: 0x%04X", val_chan.chan, val_chan.value, reg_value);
    sub_module.IndRegWrite(m_tetr_num, 0x220 + val_chan.chan, reg_value);

    return BRDerr_OK;
}

auto SrvADC::CtrlGetCompLevel(void* pDev, void* pServData, ULONG cmd, void* args) -> int
{
    decltype(auto) sub_module = ModuleCast<Cmfm414x3g>(pDev);
    decltype(auto) val_chan = ArgumentCast<BRD_ValChan>(args);
    ...
    val_chan.value = sub_module.IndRegRead(m_tetr_num, 0x220 + val_chan.chan);
    LOG("BRDctrl_ADC_GETCOMPLEVEL | ch: %d, level: %.02f V", val_chan.chan, val_chan.value);

    return BRDerr_OK;
}

///
/// \brief Установка инверсии порога старта, 0 - возрастание, 1 - спад
///
auto SrvADC::CtrlSetCompInverse(void* pDev, void* pServData, ULONG cmd, void* args) -> int
{
    decltype(auto) sub_module = ModuleCast<Cmfm414x3g>(pDev);
    decltype(auto) val_chan = ArgumentCast<BRD_ValChan>(args);
    auto ctrl = COMP_0x224 {};
    ...
    LOG("BRDctrl_ADC_SETCOMPINVERSE | ch: %d, inverse: %s", val_chan.chan, val_chan.value ? "ON" : "OFF");

    switch (val_chan.chan) {
    case 0:
        ctrl.COMP0_INV = bool(val_chan.value);
        break;
    case 1:
        ctrl.COMP1_INV = bool(val_chan.value);
        break;
    case 2:
        ctrl.COMP2_INV = bool(val_chan.value);
        break;
    case 3:
        ctrl.COMP3_INV = bool(val_chan.value);
        break;
    }
...
auto SrvADC::CtrlGetCompInverse(void* pDev, void* pServData, ULONG cmd, void* args) -> int
{
    decltype(auto) sub_module = ModuleCast<Cmfm414x3g>(pDev);
    decltype(auto) val_chan = ArgumentCast<BRD_ValChan>(args);
    auto ctrl = COMP_0x224 {};
    ...
    switch (val_chan.chan) {
    case 0:
        val_chan.value = bool(ctrl.COMP0_INV);
        break;
    case 1:
        val_chan.value = bool(ctrl.COMP1_INV);
        break;
    case 2:
        val_chan.value = bool(ctrl.COMP2_INV);
        break;
    case 3:
        val_chan.value = bool(ctrl.COMP3_INV);
        break;
    }

    LOG("BRDctrl_ADC_GETCOMPINVERSE | ch: %d, inverse: %s", val_chan.chan, val_chan.value ? "ON" : "OFF");

    return BRDerr_OK;
}

///
/// \brief Включение замены потока ADC на тестовую последовательность
///
auto SrvADC::CtrlSetTestSequence(void* pDev, void* pServData, ULONG cmd, void* args) -> int
{
    decltype(auto) sub_module = ModuleCast<Cmfm414x3g>(pDev);
    decltype(auto) enable = ArgumentCast<bool>(args);

    LOG("ADC Stream from %s", enable ? "COUNT64" : "CHIP");

    // 0x0C :  8 бит -- включение/выключение
    //        10 бит -- 0 = PSD, 1 = Count64
    sub_module.IndRegWrite(m_tetr_num, ADM2IFnr_CNTAE, (enable << 8) | (1 << 10));
...
auto SrvADC::CtrlGetJESDSyncStatus(void* pDev, void* pServData, ULONG cmd, void* args) -> int
{
    ...
    status = sub_module.SpdRead(m_tetr_num, 8, 0, (m_jesd_phy_id == 0x18) ? 0x060 : 0x038);
    return BRDerr_OK;
}
...
auto SrvADC::CtrlGetJESDDebugStatus(void* pDev, void* pServData, ULONG cmd, void* args) -> int
{
    ...
    status = sub_module.SpdRead(m_tetr_num, 8, 0, (m_jesd_phy_id == 0x18) ? 0x05C : 0x03C);
    return BRDerr_OK;
}
...
auto SrvADC::CtrlSetJESDPrbs(void* pDev, void* pServData, ULONG cmd, void* args) -> int
{
    ...
    LOG("BRDctrl_JESD_SETPRBS | 0x017 = %04Xh, Type: %d", ctrl.all, type);
...
{
    decltype(auto) sub_module = ModuleCast<Cmfm414x3g>(pDev);
    decltype(auto) status = ArgumentCast<BRD_PrbsStatus_FM414x3G>(args);
    ...
    status.count_ch2 = sub_module.IndRegRead(m_tetr_num, 0x21C);
    status.count_ch3 = sub_module.IndRegRead(m_tetr_num, 0x21D);
    return BRDerr_OK;
}
