...
///
Cmfm414x3g::Cmfm414x3g(PBRD_InitData pBrdInitData, long sizeInitData)
{
    LOG("%s", info);
    ...
    m_monitor.setup_spi(
        [this](size_t reg) -> uint8_t { return SpdRead(m_sync_tetr_num, 6, 0x49, reg); },
        [this](size_t reg, uint8_t val) -> void { SpdWrite(m_sync_tetr_num, 6, 0x49, reg, val); });
};
...
///
auto Cmfm414x3g::SideDriverInit() -> int
{
...
///
auto Cmfm414x3g::Init(uint8_t* pSubmodEEPROM, BRDCHAR* pIniString) -> S32
{
    ...
    BRDC_sprintf(m_name, _BRDC("FM414x3G_%X"), m_PID);
...
//
auto Cmfm414x3g::FindID(size_t id, size_t id_num) -> int
{
    if (base_dev == nullptr) {
    ...
    int num = GetTetrNumEx(base_dev, 0 /*srvMainIdx*/, id, id_num /*instantNum*/);
    if (num == -1) {
...
///
auto Cmfm414x3g::analysis_eeprom(uint8_t const* eeprom) -> void
{
    ...
    do {
        ...
        case SUBMODULE_CFG_TAG: { // конфигурационная структура субмодуля
            ...
            m_fosc = *reinterpret_cast<uint32_t const*>(eeprom + 6) / 1'000.0; // переводим в МГц
            m_start_level = *reinterpret_cast<uint16_t const*>(eeprom + 20) / 1'000.0; // переводим в вольты
            m_adc_min_bitrate = *reinterpret_cast<uint32_t const*>(eeprom + 12);
            m_adc_max_bitrate = *reinterpret_cast<uint32_t const*>(eeprom + 16);
            m_pll_present = *reinterpret_cast<uint8_t const*>(eeprom + 11);
            m_adc_present = *reinterpret_cast<uint8_t const*>(eeprom + 10);
        }
        }
...
/// \return     void
///
auto Cmfm414x3g::dump_icr(uint8_t* mem, size_t len) -> void
{
...
/// \return     void
///
auto Cmfm414x3g::GetDeviceInfo(BRD_Info* pInfo) -> void
{
...
/// \return     void
///
auto Cmfm414x3g::BaseInfoInit(void* pDev, DEVS_API_entry* pEntry, BRDCHAR* pName) -> void
{
...
///
auto Cmfm414x3g::RegCtrl(ULONG cmd, PDEVS_CMD_Reg pRegParam) -> LONG
{
...
///
auto Cmfm414x3g::SrvCtrl(BRD_Handle handle, ULONG cmd, PVOID arg, void* pContext) -> ULONG
{
...
/// \return     void
///
auto Cmfm414x3g::GetServ(SERV_ListItem srv_list[]) -> void
{
    ...
    for (auto i = 0U; i < m_SrvList.size(); i++) {
        ...
        LOG("- [%02u] name: %s, attr: %zX, index: %d",
            i, BARDY_STR(srv_list[i].name), size_t(srv_list[i].attr), srv_list[i].idxMain);
    }
}
...
/// \return     void
///
auto Cmfm414x3g::SetSrvList(SERV_ListItem srv_list[]) -> void
{
...
///
auto Cmfm414x3g::IndRegRead(size_t tetr_num, size_t reg_num) -> size_t
{
    DEVS_CMD_Reg param {};
    param.tetr = tetr_num;
    param.reg = reg_num;
...
/// \return     void
///
auto Cmfm414x3g::IndRegWrite(size_t tetr_num, size_t reg_num, size_t val) -> void
{
    DEVS_CMD_Reg param {};
    param.tetr = tetr_num;
    param.reg = reg_num;
...
/// \return     Значение из регистра
///
auto Cmfm414x3g::DirRegRead(size_t tetr_num, size_t reg_num) -> size_t
{
    DEVS_CMD_Reg param {};
    param.tetr = tetr_num;
    param.reg = reg_num;
...
/// \return     void
///
auto Cmfm414x3g::DirRegWrite(size_t tetr_num, size_t reg_num, size_t val) -> void
{
    DEVS_CMD_Reg param {};
    param.tetr = tetr_num;
    param.reg = reg_num;
...
/// \return     void
///
auto Cmfm414x3g::DirRegWrites(size_t tetr_num, size_t reg_num, void* buf, size_t size) -> void
{
    DEVS_CMD_Reg param {};
    // param.idxMain = index;
    param.tetr = tetr_num;
...
/// \return     Значение из регистра
///
auto Cmfm414x3g::SpdRead(size_t tetr_num, size_t dev, size_t numb, size_t addr) -> size_t
{
    auto status = size_t {};
...
/// \return     void
///
auto Cmfm414x3g::SpdWrite(size_t tetr_num, size_t dev, size_t numb, size_t addr, size_t val) -> void
{
    auto status = size_t {};
