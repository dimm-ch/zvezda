...
const U32 DU_SPDDEVICE = 0x203;
const U32 DU_SPDCTRL = 0x204;
const U32 DU_SPDADDR = 0x205;
const U32 DU_SPDDATAL = 0x206;
const U32 DU_SPDDATAH = 0x207;

const U32 DU_STATUS = 0; // (0x00) Status register
const U32 DU_DATA = 1; // (0x02) Data register
const U32 DU_CMDADR = 2; // (0x04) Address command register
const U32 DU_CMDDATA = 3; // (0x06) Data command register
...
U32 SpdRead(BRD_Handle hService, size_t tetr_num, size_t dev, size_t numb, size_t addr)
{
    ...
    do { // ожидаем готовность тетрады
        regSTAT = regReadDir(hService, tetr_num, DU_STATUS, status);
    } while ((regSTAT & 1) != 1);

    regWriteIndir(hService, tetr_num, DU_SPDDEVICE, dev);
    regWriteIndir(hService, tetr_num, DU_SPDADDR, addr);
    regWriteIndir(hService, tetr_num, DU_SPDCTRL, 1 | (numb << 4));

    do { // ожидаем готовность тетрады
        regSTAT = regReadDir(hService, tetr_num, DU_STATUS, status);
    } while ((regSTAT & 1) != 1);

    return regReadIndir(hService, tetr_num, DU_SPDDATAL, status) | regReadIndir(hService, tetr_num, DU_SPDDATAH, status) << 16;
}
...
S32 SpdWrite(BRD_Handle hService, size_t tetr_num, size_t dev, size_t numb, size_t addr, size_t val)
{
    ...
    regWriteIndir(hService, tetr_num, DU_SPDDEVICE, dev);
    regWriteIndir(hService, tetr_num, DU_SPDADDR, addr);
    regWriteIndir(hService, tetr_num, DU_SPDDATAL, val & 0xFFFF);
    regWriteIndir(hService, tetr_num, DU_SPDDATAH, val >> 16);
    regWriteIndir(hService, tetr_num, DU_SPDCTRL, 2 | (numb << 4));
