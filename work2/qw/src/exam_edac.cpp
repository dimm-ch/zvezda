...
S32 CaptureAllDac( void )
{
	...
	// {
		...
		// Открыть устройство
		//
		g_hDev[g_nDevNum] = BRD_open( rLidList.pLID[iDev], BRDopen_SHARED, NULL); // открыть устройство в разделяемом режиме
...
S32 SetAllDac( void )
{
	...
	for( ii = 0; ii < g_nDacNum; ii++ )
	{
		...
		if (BRD_errext(err) != BRDerr_OK) throw std::invalid_argument("Side-Driver parameters bad");
					  ...
					  double twiddle, double ampl, double *pPhase )
{
	...
	if( sampleWidth == 2 )
	{
		...
		for( ii=0; ii<nSamples; ii++ )
		{
            if (chanIdx<(chanNum>>1))
			    *pBuf  = (S16)floor(ampl * sin( phase ) + 0.5);
            else
                *pBuf = (S16)(2.0 * ampl * fabs(fmod(phase / PI2, 1.0) - 0.5) - ampl);
